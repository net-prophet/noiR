...
import (
	...
	sfu "github.com/net-prophet/noir/pkg/proto"
...
func main() {
	...
	client, err := c.Command(ctx)

	if err != nil {
		log.Fatalf("Error publishing stream: %v", err)
	}

	description, err := json.Marshal(peerConnection.LocalDescription().SDP)

	if err != nil {
		log.Fatalf("Error marshalling SDP: %v", err)
	}

	err = client.Send(&sfu.NoirRequest{
		Payload: &sfu.SignalRequest{
			&sfu.SignalRequest_Join{
